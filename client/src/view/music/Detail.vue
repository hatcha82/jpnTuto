...
<v-container fluid grid-list-md style="padding:5px">    
    ...
    <v-flex d-flex xs12 sm12 md12 lg6>
      <v-layout column>
      <v-card class="pa-2 mb-3" v-if="song.youtubeId">
        ...
        <div style="position: relative; padding-bottom: 56.25%;">
          <iframe style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;" :src="'https://www.youtube.com/embed/'+ song.youtubeId +'?autoplay=1&playsinline=1'" frameborder="0" gesture="media" allow="autoplay;encrypted-media" ></iframe>
        </div>
      <br>
    ...
    <v-flex d-flex xs12 sm12 md6 lg6>
      <div>
      <v-card  class="mb-3" v-if="artistSongs && artistSongs.length > 0">
         <v-card-title class="primary">
            <span class="white--text">{{song.artist}}의 다른노래</span>
          </v-card-title>
          <v-container>
          <ArtistMusicList ref="artistMusicList" :songs="artistSongs" :currentSong="song"/>
          </v-container>
      </v-card>
      <v-card  class="mt-0">
         <v-card-title class="primary">
